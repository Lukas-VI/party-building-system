const path = require('node:path');
const dotenv = require('dotenv');

dotenv.config({ path: path.join(__dirname, '..', '.env') });

/**
 * Parse the comma-separated CORS origin allowlist from environment configuration.
 */
function parseOrigins(raw) {
  return (raw || '')
    .split(',')
    .map((item) => item.trim())
    .filter(Boolean);
}

function resolvePublicBaseUrl() {
  if (process.env.PUBLIC_BASE_URL) return process.env.PUBLIC_BASE_URL;
  if (process.env.NODE_ENV === 'production') {
     console.warn('[warn] PUBLIC_BASE_URL 未配置，附件链接可能无法访问。请在 server/.env 中设置 PUBLIC_BASE_URL');
     return 'http://127.0.0.1:3000';
  }
  return 'http://127.0.0.1:3000';
}

const env = {
  NODE_ENV: process.env.NODE_ENV || 'development',
  PORT: process.env.PORT || '3000',
  JWT_SECRET: process.env.JWT_SECRET || 'change-this-secret',
  DB_HOST: process.env.DB_HOST || '127.0.0.1',
  DB_PORT: process.env.DB_PORT || '3306',
  DB_NAME: process.env.DB_NAME || 'party_building_app',
  DB_USER: process.env.DB_USER || 'root',
  DB_PASSWORD: process.env.DB_PASSWORD || '',
  UPLOAD_DIR: process.env.UPLOAD_DIR || path.join(__dirname, '..', 'uploads'),
  PUBLIC_BASE_URL: resolvePublicBaseUrl(),
  CORS_ORIGINS: parseOrigins(process.env.CORS_ORIGINS || 'http://localhost:5173,http://127.0.0.1:5173'),
  ALLOW_ALL_CORS: process.env.ALLOW_ALL_CORS === 'true' || process.env.NODE_ENV === 'development',
  WECHAT_APP_ID: process.env.WECHAT_APP_ID || '',
  WECHAT_APP_SECRET: process.env.WECHAT_APP_SECRET || '',
  WECHAT_SERVICE_APP_ID: process.env.WECHAT_SERVICE_APP_ID || process.env.WECHAT_APP_ID || '',
  WECHAT_SERVICE_APP_SECRET: process.env.WECHAT_SERVICE_APP_SECRET || process.env.WECHAT_APP_SECRET || '',
  WECHAT_SERVICE_REDIRECT_URI: process.env.WECHAT_SERVICE_REDIRECT_URI || '',
  WECHAT_BIND_SUCCESS_TEMPLATE_ID: process.env.WECHAT_BIND_SUCCESS_TEMPLATE_ID || 'wnZT-XnyMeh1UW0y7Nscz2nVnNnyVZtNCNTtQZunKlw',
  WECHAT_UNBIND_SUCCESS_TEMPLATE_ID: process.env.WECHAT_UNBIND_SUCCESS_TEMPLATE_ID || '2oVVJm20kfDLopQdJFyAEoYn8diJzwIKO1nxH5JXvbo',
  WECHAT_REGISTRATION_APPROVAL_TEMPLATE_ID: process.env.WECHAT_REGISTRATION_APPROVAL_TEMPLATE_ID || 'Uu76NJ-hIlWi_A8enlHiq8xIfQdlG2-AKfd6j-f3dIg',
  WECHAT_WORKFLOW_APPROVAL_TEMPLATE_ID: process.env.WECHAT_WORKFLOW_APPROVAL_TEMPLATE_ID || 'zkxuUQtXptzR9PhxdJppSpotFCrLLo9CZkpIKb3gS8g',
  WECHAT_PROXY_URL: process.env.WECHAT_PROXY_URL || '',
  WECHAT_PROXY_TOKEN: process.env.WECHAT_PROXY_TOKEN || '',
  WECHAT_SESSION_SECRET: process.env.WECHAT_SESSION_SECRET || process.env.JWT_SECRET || 'change-this-secret',
  WECHAT_DEFAULT_REDIRECT_PATH: process.env.WECHAT_DEFAULT_REDIRECT_PATH || '/wx-app/',
  WECHAT_VERIFY_FILEPATH: process.env.WECHAT_VERIFY_FILEPATH || '',
  TEST_DEFAULT_PASSWORD_HINT: process.env.TEST_DEFAULT_PASSWORD_HINT || '当前测试环境统一密码：123456',
};

module.exports = {
  env,
};
