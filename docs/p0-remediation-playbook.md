...
本文记录当前阶段采用的整改方法，用于约束安全缺陷、25 步流程和可维护性问题的后续处理。
...
- PC 端：`admin-desktop/` 桌面后台，挂载路径 `/admin-desktop/`。
- 后端：`server/` Node API。
- 流程：当前按完整 25 步办理闭环开放；历史前 12 步 MVP 口径仅作为早期整改记录保留。
- 不再维护小程序入口、页面或依赖。
...
- `STEP_01` 到 `STEP_25` 是当前可办理范围。
- `STEP_13` 到 `STEP_25` 必须按 `docs/workflow-step-action-mapping.md` 区分提交类和通知类。
- 当前节点办理前必须满足上一流程节点已通过。
